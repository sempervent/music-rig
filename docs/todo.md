...
|---|---|---|
| RIG-001 | Physically verify DIRTY and SPACE pedal topologies against docs | Unlocks confident playing; catches doc drift immediately |
| RIG-002 | Record PB-B normalization mode for every populated pair | Highest-value patchbay unknown; quick rack check |
| RIG-003 | Trace non-clean A/B/Y legs for acoustic, bass, and electric | Completes clean-path story without buying anything |
...
| RIG-032 | Design and implement RC-600 integration | RC-600 acquisition (wishlist BUY LATER). Done when audio path, patchbay access, MIDI clock, Ableton relationship, foot-control roles, RC-1 post-integration role, emergency-stop/recovery, and CURRENT docs are updated after physical install — no speculative CURRENT wiring beforehand. |
...
Next free ID after this expansion: **RIG-049**.
