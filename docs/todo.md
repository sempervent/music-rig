...
| RIG-015 | Define and run known-good baseline smoke test | Studio ops | P1 | READY | RIG-001 helpful | Checklist + dated pass/fail proving clean captures, AUX loop, and KAOSS path; distinguishes “broken” from “booted differently” | Optional verification — PLAYABLE without re-running smoke campaign. |
...
|---|---|---|---|
| RIG-002 | Inspect and record normal / half-normal / thru for every populated PB-B pair | — | Upper 9–12 / lower 33–36 included if physically checkable Closed via Q-008 HUMAN answer + CURRENT all-normal reconcile (Stage 27). |
...
| RIG-031 | Confirm Ableton as default master clock in practice | — | — |
| RIG-037 | Document and verify final physical MIDI topology | — | Owned CME gear — operate, don’t repurchase. Stage 27: device-level fanout accepted; Ableton Track/Sync/Remote per-port, U6MIDI filter programming, feedback/All Notes Off, MIDI-monitor remain optional later. |
